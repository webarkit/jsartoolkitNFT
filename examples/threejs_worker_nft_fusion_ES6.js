...
export default function start(
    ...
    const load = function () {
        ...
        worker = new Worker("../js/artoolkitNFT_nft_fusion_ES6.worker.js");
        ...
        let gyroData = {gx: 0.0, gy: 0.0, gz: 0.0};

        window.addEventListener('devicemotion', (event) => {

            // Calculate delta time
            const dt = 0.016; // Use real delta time in production!
            ...
            if (gyro) {
                ...
                const gx = gyro.alpha * (Math.PI / 180);
                const gy = gyro.beta * (Math.PI / 180);
                const gz = gyro.gamma * (Math.PI / 180);

                gyroData = {gx: gx, gy: gy, gz: gz}

                //worker.postMessage({type: 'gyroMsg', gyroData: gyroData })
            }
        });

        worker.postMessage({type: 'gyroMsg', gyroData: JSON.stringify(gyroData)})
