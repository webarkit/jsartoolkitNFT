...
emscripten::val ARToolKitNFT::getNFTMarkerInfo(int markerIndex) {
  ...
  if (this->threadHandle) {
    ...
    if (this->detectedPage == -2) {
      trackingInitStart(this->threadHandle, this->videoLuma);
      this->detectedPage = -1;
    }
...
int ARToolKitNFT::setupAR2() {
  if ((this->ar2Handle = ar2CreateHandle(this->paramLT, this->pixFormat, AR2_TRACKING_DEFAULT_THREAD_NUM)) ==
      NULL) {
...
ARToolKitNFT::addNFTMarkers(std::vector<std::string> &datasetPathnames) {
  ...
  this->threadHandle = trackingInit(this->kpmHandle);
